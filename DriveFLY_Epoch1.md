...
如果需要测试正则的话，可以试试这些网站：https://regex101.com/、https://deerchao.cn/tools/wegester/
...
有幸玩过几次`Make`，最好的当然是直接用包管理器，但一些较新的软件包管理器里头没有，就只能自己动手了。

比较典型的是 Nginx，之前做 Docker 反代需要修改 HTTP 头，需要安装额外的扩展：`headers-more-nginx-module`，包管理器不提供，只能把对应版本的 Nginx 从官网拉下来，找到这个扩展的源代码，拉下来，然后 Make，虽然说整个流程很通畅，但这个感觉还是相当酸爽。

Makefile 我是从来没看过的，没资格看，毕竟 UNIX 用得都不多。Make 在重复构建时会忽略没有变更的部分，缩短构建时长。

[语义化版本](https://semver.org/lang/zh-CN/)是个规范，描述了在开发过程中版本迭代应该如何变更版本号，收藏留做参考。锁这部分 npm 经验丰富，`~`锁定次版本号，`^`锁定主版本号，还有复复杂杂的大于号小于号已经`lock`文件各种规则。

CI 目前在用，主要是在博客仓库更新后自动重新发布，东西是好东西。
...
正则除了用于常见的“正向匹配”，即通过指定的表达式查找我想要的内容外，也支持“反向匹配”，也可以说成排除过滤。

考虑一个场景，我希望不匹配所有包含“提示”二字的条目，可以这么写：`^(?!.*提示).*$`

这被称之为“负向前瞻”，也叫“零宽度负预测先行断言”，用于断言此位置的后面不能匹配表达式，奇奇怪怪的名字。
...
这个场景用在哪呢，用在著名 V 牌盾构机的订阅过滤规则中，C 牌盾构机提供过滤正则规则，而 V 牌只能通过这种方式来实现排除过滤。
...
在了解流媒体解锁的过程中意外学习了很多关于代理的新知识，春节后的小例会简要做一次分享。
